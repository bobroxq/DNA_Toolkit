import random
import dna_toolkit.dna_toolkit
from dna_toolkit.utilities import colored
from dna_toolkit.structures import NUCLEOTIDES
...
def main(DNAStr):
    ...
    print(
        f"[6] + GC Content in Subsection k=5: {dna_toolkit.gc_content_sub(DNAStr, k=5)}\n"
    )

    print(
        f"[7] + Aminoacid Sequence from DNA: {dna_toolkit.translate_seq(DNAStr, 0)}\n"
    )
